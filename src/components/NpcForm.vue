<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-gray-800 p-8 rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
      <h2 class="text-2xl font-bold mb-4 text-amber-500">{{ isEditing ? 'Editar NPC' : 'Novo NPC' }}</h2>
      <form @submit.prevent="handleSubmit">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <label for="campaign" class="block text-gray-300 text-sm font-bold mb-2">Campanha:</label>
            <select id="campaign" v-model="form.campaign_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline bg-gray-700 border-gray-600 text-white" required>
        ...
        <div class="flex justify-end gap-4 mt-6">
          ...
          <button type="submit" class="bg-amber-600 hover:bg-amber-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
            {{ isEditing ? 'Salvar Alterações' : 'Adicionar NPC' }}
          </button>
        </div>
      </form>
    </div>
