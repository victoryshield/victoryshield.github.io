<template>
  <div class="md:col-span-1 p-4 rounded-lg shadow bg-white dark:bg-slate-800 max-h-[600px] overflow-y-auto relative">
    ...
    <div v-if="showCampaignFilter" class="mb-4">
      ...
      <select id="campaign-filter" :value="selectedCampaignId" @change="$emit('update:selectedCampaignId', $event.target.value)" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-900 dark:text-white leading-tight focus:outline-none focus:shadow-outline bg-white dark:bg-slate-700 border-gray-300 dark:border-slate-600">
